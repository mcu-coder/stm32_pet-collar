...
void uart_init(u32 bound)
{
   ...
   //USART 初始化设置
	...
	CLR_Buf();//清空串口接收缓存
	GPS_ClearBuffer();// 替换原clrStruct()：清空GPS模块缓冲区
}
    ...
    if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) {
        ...
        if(Res == '\n') {
            ...
            if (gga_ptr || rmc_ptr || gll_ptr) {
                memset(gps_data.buf, 0, GPS_BUF_MAX_LEN);
...
void CLR_Buf(void)                           // 串口缓存清理（保留原功能）
{
	memset(USART_RX_BUF, 0, USART_REC_LEN);      //清空串口接收缓存
    point1 = 0;                    
}
